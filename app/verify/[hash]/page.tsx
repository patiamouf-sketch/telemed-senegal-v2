  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="p-6 sm:p-8 rounded-[28px] bg-white border border-slate-200/90 shadow-2xl relative overflow-hidden min-h-[580px] sm:min-h-[660px] flex flex-col justify-between">
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between border-b-1.5 border-slate-900 pb-3 gap-3">
              <div className="flex-1">
                ...
                <div className="text-[11px] text-slate-600 mt-1 leading-snug">
                  <strong className="text-slate-900 font-bold text-xs">{prescription.doctorName}</strong>
                  {prescription.doctorSpeciality && (
                    <span> • <span className="font-medium text-[#3B82F6]">{prescription.doctorSpeciality}</span></span>
                  )}
                  {isOnmsRegistered ? (
                    <span className="font-mono text-emerald-800 font-bold text-[10px]"> • N° ONMS : {prescription.doctorOnms}</span>
                  ) : (
                    <span className="text-[10px] text-slate-500 italic"> • Praticien diplômé d'État</span>
                  )}
                  <p className="text-[10px] text-slate-500 mt-0.5">{prescription.doctorClinic || 'Cabinet Médical'} • {prescription.doctorCity || 'Sénégal'}</p>
                </div>
              </div>
            ...
            {/* Patient / Beneficiary Identity Box Compact */}
            <div className="p-2.5 sm:p-3 rounded-[14px] bg-slate-50 border border-slate-200 text-xs grid grid-cols-1 sm:grid-cols-3 gap-2 items-center">
              <div>
                <span className="text-slate-400 text-[9px] block font-bold uppercase tracking-wider">Patient(e) Bénéficiaire :</span>
                <div className="flex items-baseline gap-1.5 flex-wrap">
                  <strong className="text-[#0F172A] font-extrabold text-xs">{prescription.patientName}</strong>
                  <span className="text-slate-600 text-[11px]">({prescription.patientGender === 'F' ? 'Femme' : 'Homme'}, {prescription.patientAge} ans)</span>
                </div>
              </div>
              <div>
                <span className="text-slate-400 text-[9px] block font-bold uppercase tracking-wider">Contact / Tél :</span>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-slate-900 font-bold text-xs">{prescription.patientPhone}</span>
                  {(prescription.beneficiaryWeight || prescription.patientWeight) && (
                    <span className="text-[10px] text-[#3B82F6] font-bold">({prescription.beneficiaryWeight || prescription.patientWeight})</span>
                  )}
                </div>
              </div>
              <div>
                <span className="text-slate-400 text-[9px] block font-bold uppercase tracking-wider">Résidence / Adresse :</span>
                <span className="text-slate-800 font-medium text-xs truncate block">{prescription.beneficiaryAddress || prescription.patientAddress || 'Sénégal'}</span>
              </div>
            </div>
          ...
          {/* Official Stamp & QR Code (Anchored at the bottom) */}
          <div className="pt-5 mt-auto border-t-2 border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
