...
- [x] **Étape 1 : Mettre à jour `lib/firebase.ts` pour supporter `ignoreUndefinedProperties: true`**
...
- [x] **Étape 2 : Ajouter la fonction `checkFirestoreHealth` dans `lib/firebase.ts`**
...
- [x] **Étape 1 : Corriger les permissions de lecture sur `match /doctors/{doctorId}` dans `firestore.rules`**
...
- [x] **Étape 1 : Importer `deleteField` dans `adminService.ts`**
Ajouter `deleteField` aux imports de `firebase/firestore`.

- [x] **Étape 2 : Remplacer `undefined` par `deleteField()` dans `approveDoctor` et `unbanDoctor`**
...
- [x] **Étape 3 : Supprimer les écritures doublons sur `targetEmail`**
...
- [x] **Étape 1 : Créer la fonction utilitaire `cleanFirestoreData(obj)`**
Une fonction récursive pure qui élimine toute propriété `undefined` d'un objet avant envoi à Firestore.

- [x] **Étape 2 : Sécuriser `completeConsultation`**
...
- [x] **Étape 3 : Sécuriser `createDoctorProfile` et `addPatientToQueue`**
...
- [x] **Étape 1 : Exécuter un script PowerShell de vérification complète**
Contrôler :
...
- [x] **Étape 2 : Générer l'artefact walkthrough et commit final**
```bash
